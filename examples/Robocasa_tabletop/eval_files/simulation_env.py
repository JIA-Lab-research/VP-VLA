        ...
        while completed_episodes < config.n_episodes:
            ...
            for env_idx in range(config.n_envs):
                ...
                if terminations[env_idx] or truncations[env_idx]:
                    ...
                    # Force model reset for the next episode to properly save
                    # overlay videos and reset VLM/gripper state tracking.
                    # This is needed because for tasks with constant task
                    # descriptions (e.g., base PnP tasks), the model's reset()
                    # is never triggered by task_description changes.
                    if hasattr(self.model, 'reset') and getattr(self.model, 'task_description', None) is not None:
                        self.model.reset(self.model.task_description)
            obs = next_obs
        # Save any remaining overlay video from the last episode
        if hasattr(self.model, 'finalize_overlay_video'):
            self.model.finalize_overlay_video()
        # Clean up
        self.env.reset()
        self.env.close()
        self.env = None
...
class Args:
    ...
    #################################################################################################################
    # Overlay video (optional, for debugging visual prompts)
    #################################################################################################################
    save_overlay_video: bool = False
    overlay_video_dir: str = "./overlay_videos"

    #################################################################################################################
    # VLM-based task decomposition
    #################################################################################################################
    vlm_decompose_subtasks: bool = False  # If True, use VLM to decompose task into subtasks instead of rule-based regex
