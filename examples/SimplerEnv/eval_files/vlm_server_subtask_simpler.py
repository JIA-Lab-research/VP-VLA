# VLM Subtask Detection Server for SimplerEnv (single gripper)
#
# Adapted from Robocasa's vlm_server_subtask.py.
# Key difference: SimplerEnv robots (widowx, google_robot) have a SINGLE
# gripper, so the prompt and API use a scalar gripper state ("OPEN"/"CLOSED")
# instead of the dual left/right gripper dict used by Robocasa's GR1 robot.
...
def build_subtask_detection_prompt(
    ...
    """
    Build subtask detection prompt for a single-gripper robot.

    Args:
        gripper_state: "OPEN" or "CLOSED" (single scalar, not a dict)
...
GRIPPER STATE CHANGE DETECTED:
GRIPPER: {previous_gripper_state} -> {gripper_state}
...
def build_task_decomposition_prompt(task_description: str) -> str:
    """Build a prompt for decomposing a task into sequential subtasks (text-only)."""
...
class VLMModel:
    """Wrapper for Qwen3-VL model for single-gripper subtask detection."""
    ...
    def decompose_task(
        ...
        """Decompose a task description into sequential subtasks (text-only, no images)."""
...
class VLMServer:
    """Websocket server for single-gripper VLM subtask detection."""
    ...
    def __init__(self, model: VLMModel, host: str = "0.0.0.0", port: int = 10102, idle_timeout: int = -1):
        ...
        self._metadata = {"service": "vlm_subtask_detection_simpler"}
    ...
    async def run(self):
        async with websockets.asyncio.server.serve(
            ...
            logging.info(f"VLM SimplerEnv server listening on {self._host}:{self._port}")
    ...
    def _route_message(self, msg: dict) -> dict:
        ...
        if mtype == "ping":
            ...
            try:
                ...
                # Single gripper: expect a string, not a dict
                gripper_state = msg.get("gripper_state", "OPEN")
...
def build_argparser():
    parser = argparse.ArgumentParser(description="VLM Subtask Detection Server (SimplerEnv, single gripper)")
    parser.add_argument(
        "--model-path", type=str,
...
def main():
    ...
    logging.info(f"Starting VLM SimplerEnv server (host: {hostname}, ip: {local_ip})")

    model = VLMModel(model_path=args.model_path, device=device)
    server = VLMServer(model=model, host="0.0.0.0", port=args.port, idle_timeout=args.idle_timeout)
